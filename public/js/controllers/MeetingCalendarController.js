...
app.controller('MeetingCalendarController',
    function MeetingCalendarController($scope,$compile, $http) {
        ...
        $http.get("/api/v1/meeting").then(function (response) {
            ...
            {
                $scope.events[i] = {id:response.data.meetings[i].id, title: response.data.meetings[i].title,start: new Date(response.data.meetings[i].created_at), end: new Date(response.data.meetings[i].created_at),allDay: false};
            }
        });
